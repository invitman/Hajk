    ...
    proxy: {
      proxy1: {
        options: {
          ...
          router: {
            'localhost/mapservice': 'localhost:53725/', // Create a virtual route on 9000 that redirects to IIS' /mapservice. NB: Make sure to have IIS running! //localhost/mapservice must be available.
            //'localhost/postProxy.aspx': 'localhost/postProxy.aspx', // proxy runs on IIS too, so it isn't available on port 9000. Since it isn't in /mapservice subdir, we need to be explicit about it
